...
export default function BookDetails(props) {
  ...
  const FetchData = () => {
    return useQuery('data', async () => {
      const response = await fetch(
        `https://www.googleapis.com/books/v1/volumes?q=${bookTitle}+inauthor:${author}`
      );
  ...
  Books.map(book => {
    ...
    if (
      ...
      !volumeInfo.hasOwnProperty('title') ||
      !volumeInfo.hasOwnProperty('language') ||
      !volumeInfo.hasOwnProperty('authors')
    ) {
      return null;
    }
    let check = author;
